...
public class LockCommand implements SlashCommandCreateListener {


    @Override
    public void onSlashCommandCreate(SlashCommandCreateEvent event) {
        SlashCommandInteraction interaction = event.getSlashCommandInteraction();
        if (interaction.getFullCommandName().equals("lock")) {
            ...
            if (user.getRoles(server).contains(ceo)
                ...
                try {
                    ...
                    perms.setDenied(PermissionType.SEND_MESSAGES);
                    ...
                    interaction.createImmediateResponder().setContent("El canal ha sido bloqueado.").respond();
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            } else {
